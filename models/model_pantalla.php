...
if ($_POST['accion'] == "Verturnos") {
...
#if ($_POST['accion'] == "Verturnos") {
    require_once('../config/conexion.php');
    $query = mysqli_query($mysqli, "SELECT t.turno, t.modulo, c.pnombre, c.papellido,
        CASE 
        ...
        END AS estado 
        FROM db_turnos t
        LEFT JOIN db_clientes c ON t.id_cliente = c.id_cliente
        WHERE t.estado_turno IN ('M', 'S', 'F') AND DATE_FORMAT(t.tiempo_ingreso, '%Y-%m-%d') = CURDATE() 
    ...
    $arrResponse = (!empty($datos)) ? array('status' => true, 'data' => $datos) : array('status' => false, 'data' => []);
